...
class GitWatcher:
        ...
        if self._repo is None:
            if self.cache_dir.exists() and (self.cache_dir / '.git').exists():
                try:
                    ...
                    self.logger.info("Loaded existing repository", path=str(self.cache_dir))
                except InvalidGitRepositoryError:
                    self.logger.warning("Invalid git repository, will re-clone", path=str(self.cache_dir))
                    self._repo = None
    ...
    def get_changed_files(self, old_commit: str, new_commit: str) -> Tuple[List[str], List[str], List[str]]:
        """Get lists of added, modified, and deleted markdown files between commits.
        ...
        Returns:
            Tuple of (added_files, modified_files, deleted_files) - all markdown files only
        """
        try:
            # Get commit objects
            ...
            for item in diff:
                ...
                # Note: 'R' (renamed) and 'T' (type changed) are handled as modify
    ...
    def get_file_commit_info(self, file_path: str) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.debug("File commit info", 
                            file=file_path, 
            ...
            self.logger.error("Failed to get file commit info", 
                            file=file_path, 
                            error=str(e))
            raise
    ...
    def sync_repository(self) -> Tuple[str, bool]:
        ...
        try:
            ...
            has_changes = self.has_changes(old_commit if old_commit != new_commit else None, new_commit)
    ...
    def get_github_file_url(self, file_path: str) -> str:
        ...
        self.logger.debug("Generated GitHub URL", 
                         file=file_path, 
                         url=file_url)
