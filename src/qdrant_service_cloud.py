...
class QdrantCloudService(VectorServiceInterface):
    ...
    def create_collection(self, collection_name: str, vector_size: int) -> bool:
        ...
        try:
            ...
            self.logger.info("Collection created successfully", collection_name=collection_name)
            return True
            
        except UnexpectedResponse as e:
            if e.status_code == 401:
                raise QdrantCloudAuthenticationError("Authentication failed: Invalid API key")
    ...
    def delete_collection(self, collection_name: str) -> bool:
        ...
        try:
            ...
            self.logger.info("Collection deleted successfully", collection_name=collection_name)
            return True
            ...
            if e.status_code == 401:
                ...
                self.logger.info("Collection does not exist", collection_name=collection_name)
                return True
            else:
                raise QdrantCloudError(f"Qdrant Cloud operation failed: {e}")
        except ResponseHandlingException as e:
    ...
    def upsert_points(self, collection_name: str, points: List[Dict[str, Any]]) -> bool:
        ...
        for i, point in enumerate(points):
            if "id" not in point or "vector" not in point:
                raise QdrantCloudError(f"Invalid point structure at index {i}: missing 'id' or 'vector'")
        ...
        try:
            ...
            if isinstance(e, (QdrantCloudError, QdrantCloudAuthenticationError, QdrantCloudConnectionError)):
                raise
            else:
    ...
    def _upsert_chunked(self, collection_name: str, points: List[Dict[str, Any]]) -> bool:
