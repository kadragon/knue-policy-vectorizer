...
class ConfigurationManager:
    ...
    def __init__(self, config_dir: str = "./config"):
        ...
        # Create directories
        for directory in [self.config_dir, self.templates_dir, self.profiles_dir, 
                         self.backups_dir, self.secrets_dir]:
    ...
    def _ensure_default_templates(self):
        ...
        default_templates = [
            ...
            ConfigTemplate(
                ...
                required_env_vars=["OPENAI_API_KEY", "QDRANT_CLOUD_URL", "QDRANT_API_KEY"],
    ...
    def validate_config(self, config: Config) -> Dict[str, Any]:
        ...
        if config.embedding_provider == EmbeddingProvider.OPENAI:
            ...
            if config.openai_model == "text-embedding-3-small" and config.vector_size != 1536:
            ...
            elif config.openai_model == "text-embedding-3-large" and config.vector_size != 3072:
                validation_result["warnings"].append(
        ...
        if config.vector_provider == VectorProvider.QDRANT_CLOUD:
            ...
            if not config.qdrant_cloud_url:
                ...
                validation_result["warnings"].append("Qdrant Cloud URL should use HTTPS")
    ...
    def save_template(self, template: ConfigTemplate) -> bool:
        ...
        try:
            ...
            self.logger.info("Template saved", name=template.name, file=str(template_file))
            return True
            
        except Exception as e:
            self.logger.error("Failed to save template", name=template.name, error=str(e))
            return False
    ...
    def create_config_from_template(self, template_name: str, 
                                  env_overrides: Dict[str, str] = None) -> Optional[Config]:
        ...
        for var in template.required_env_vars:
            if var not in os.environ and (env_overrides is None or var not in env_overrides):
                missing_vars.append(var)
        
        if missing_vars:
            self.logger.error("Missing required environment variables", 
                            variables=missing_vars, template=template_name)
            return None
    ...
    def list_profiles(self, environment: Optional[str] = None) -> List[ConfigProfile]:
        ...
        for profile_file in self.profiles_dir.glob("*.json"):
            try:
                ...
                if profile and (environment is None or profile.environment == environment):
                    profiles.append(profile)
            except Exception as e:
                self.logger.warning("Failed to load profile", 
                                  file=str(profile_file), error=str(e))
        ...
        try:
            ...
            if expected_hash:
                ...
                if actual_hash != expected_hash:
                    self.logger.warning("Backup integrity check failed", 
                                      file=backup_file)
            ...
            self.logger.error("Failed to restore backup", file=backup_file, error=str(e))
            return None
    ...
    def decrypt_sensitive_config(self, encrypted_values: Dict[str, str]) -> Dict[str, str]:
    ...
    def export_config(self, config: Config, format: str = "json", 
