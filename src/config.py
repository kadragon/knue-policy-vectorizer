...
class Config:
    ...
    @classmethod
    def _get_env_int(cls, canonical_key: str, legacy_key: Optional[str] = None, default_value: int = 0) -> int:
    ...
    @classmethod
    def _get_env_str(cls, canonical_key: str, legacy_key: Optional[str] = None, default_value: str = "") -> str:
    ...
    def from_env(cls) -> "Config":
        ...
        qdrant_collection = cls._get_env_str("COLLECTION_NAME", "QDRANT_COLLECTION", cls.qdrant_collection)
        vector_size = cls._get_env_int("VECTOR_SIZE", default_value=cls.vector_size)

        ollama_url = os.getenv("OLLAMA_URL", cls.ollama_url)
        embedding_model = cls._get_env_str("OLLAMA_MODEL", "EMBEDDING_MODEL", cls.embedding_model)

        max_workers = cls._get_env_int("MAX_WORKERS", default_value=cls.max_workers)
        max_document_chars = cls._get_env_int("MAX_DOCUMENT_CHARS", default_value=cls.max_document_chars)
        ...
        chunk_threshold = cls._get_env_int("CHUNK_THRESHOLD", default_value=cls.chunk_threshold)
        chunk_overlap = cls._get_env_int("CHUNK_OVERLAP", default_value=cls.chunk_overlap)
