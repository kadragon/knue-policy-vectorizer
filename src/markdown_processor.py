...
class MarkdownProcessor:
    ...
    def estimate_token_count(self, content: str, use_embedding_service: bool = True) -> int:
        ...
        if use_embedding_service:
            try:
                ...
                self.logger.debug("Token count estimated using embedding service", 
                ...
                self.logger.warning("Failed to use embedding service for token estimation, falling back to simple method", 
                                  error=str(e))
        ...
        self.logger.debug("Token count estimated using character method", 
    ...
    def _split_oversized_chunk(self, chunk: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        self.logger.info("Split oversized chunk", 
                        original_tokens=chunk['tokens'],
                        sub_chunks=len(sub_chunks),
                        avg_tokens=sum(c['tokens'] for c in sub_chunks) // len(sub_chunks) if sub_chunks else 0)
    ...
    def chunk_markdown_content(self, content: str, max_tokens: int = None) -> List[Dict[str, Any]]:
        ...
        def create_chunk(start_line: int, end_line: int, include_overlap: bool = True) -> Dict[str, Any]:
        ...
        while current_line < len(lines):
            ...
            for line_idx in range(current_line, len(lines)):
                ...
                if not stripped:  # Empty line - good break point
                    ...
                    last_good_break = line_idx + 1
                elif any(stripped.startswith(marker) for marker in ['---', '***', '___']):  # Horizontal rule
                    last_good_break = line_idx + 1
            ...
            if (last_good_break > current_line and 
                last_good_break <= best_break_point and
                best_break_point - current_line > 5):  # Don't use if chunk would be tiny
        ...
        for chunk in chunks:
            if chunk['tokens'] > self.max_tokens:
                self.logger.warning("Chunk exceeds embedding token limit, splitting further", 
        ...
        self.logger.info("Content chunked with 800/200 strategy", 
                        total_chunks=len(validated_chunks),
                        original_tokens=total_original_tokens,
                        total_chunk_tokens=total_chunk_tokens,
                        avg_tokens_per_chunk=total_chunk_tokens // len(validated_chunks) if validated_chunks else 0,
                        chunks_with_overlap=sum(1 for c in validated_chunks if c['has_context_overlap']))
    ...
    def create_document_for_vectorization(self,
                                        ...
                                        file_path: str,
                                        commit_info: Dict[str, str],
                                        github_url: str) -> List[Dict[str, Any]]:
        ...
        if processed_content['needs_chunking'] and processed_content['chunks']:
            ...
            for chunk in processed_content['chunks']:
                ...
                chunk_metadata['document_id'] = f"{original_id}_chunk_{chunk['chunk_index']}"
            ...
            self.logger.info("Created chunked documents for vectorization",
                            file_path=file_path,
            ...
            self.logger.info("Created single document for vectorization",
