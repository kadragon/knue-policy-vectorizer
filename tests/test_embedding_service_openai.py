...
class TestOpenAIEmbeddingService:
    ...
    def test_service_initialization(self):
        ...
        with patch('src.embedding_service_openai.OpenAI') as mock_openai:
            ...
            assert service.embedding_dimension == 3072  # text-embedding-3-large dimension

    def test_model_dimension_mapping(self):
        """Test that model dimensions are correctly mapped"""
        with patch('openai.OpenAI'):
            # text-embedding-3-small
            service = OpenAIEmbeddingService(api_key="test", model="text-embedding-3-small")
            assert service.embedding_dimension == 1536
            
            # text-embedding-3-large
            service = OpenAIEmbeddingService(api_key="test", model="text-embedding-3-large")
            assert service.embedding_dimension == 3072
            
            # text-embedding-ada-002 (legacy)
            service = OpenAIEmbeddingService(api_key="test", model="text-embedding-ada-002")
            assert service.embedding_dimension == 1536
    ...
    def test_generate_embedding_with_token_validation(self):
        ...
        with patch('src.embedding_service_openai.OpenAI') as mock_openai:
            ...
            with pytest.raises(OpenAIEmbeddingError, match="exceeds maximum token limit"):
