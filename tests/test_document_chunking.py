...
class TestDocumentChunking:
    ...
    def test_chunking_basic_functionality(self):
        ...
        long_content = """# 긴 문서 제목
        
첫 번째 섹션의 내용입니다. """ + "이것은 매우 긴 내용입니다. " * 1000 + """

## 두 번째 섹션

두 번째 섹션의 내용입니다. """ + "또 다른 긴 내용입니다. " * 1000
...
첫 번째 섹션의 내용입니다. """ + "적당한 길이의 내용. " * 100 + """

## 섹션 A

섹션 A의 내용입니다. """ + "섹션 A 관련 내용. " * 100 + """

### 하위 섹션 A.1

하위 섹션의 내용입니다. """ + "하위 내용. " * 100 + """

## 섹션 B

섹션 B의 내용입니다. """ + "섹션 B 관련 내용. " * 100
        ...
        for chunk in chunks:
            ...
            if any(line.strip().startswith('#') for line in content_lines):
                ...
                header_lines = [line for line in content_lines if line.strip().startswith('#')]
...
본 조항은 정책의 목적과 적용 범위를 정의합니다. """ + "목적 관련 상세 내용. " * 150 + """

## 제2조 정의

이 정책에서 사용되는 용어의 정의는 다음과 같습니다. """ + "정의 관련 상세 내용. " * 150 + """

## 제3조 절차

정책 실행을 위한 절차는 다음과 같습니다. """ + "절차 관련 상세 내용. " * 150
...
### 제3조 조직 구조
대학교의 조직은 다음과 같이 구성됩니다. """ + "조직 관련 상세 내용. " * 200
...
청킹이 필요하지 않을 정도로 짧습니다."""
    ...
    def test_chunking_handles_empty_content(self):
        ...
        # Whitespace only
        whitespace_content = "   \n\n   \n  "
        whitespace_chunks = processor.chunk_markdown_content(whitespace_content, max_tokens=800)
...
""" + "두 번째 섹션 내용. " * 100
        ...
        for chunk in chunks:
            if chunk['section_title']:
                ...
                headers_in_chunk = [line.strip() for line in chunk['content'].split('\n') 
                                  if line.strip().startswith('#')]
                if headers_in_chunk:
                    # The section title should be related to headers in the chunk
                    assert any(chunk['section_title'] in header.lstrip('#').strip() 
...
""" + "English content here. " * 50
        ...
        for chunk in chunks:
            ...
            assert abs(estimated_tokens - reported_tokens) <= max(50, estimated_tokens * 0.2)
...
이것은 제{i+1}조의 내용입니다. """ + f"섹션 {i+1} 관련 상세 내용입니다. " * 50
