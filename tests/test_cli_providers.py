...
class TestCLIProviders:
    ...
    def test_configure_providers_with_validation_errors(self):
        ...
        assert result.exit_code == 0
        assert "Invalid provider" in result.output or "Configuration saved" in result.output
    ...
    def test_sync_with_provider_options(self):
        ...
        with patch('src.sync_pipeline.SyncPipeline') as mock_pipeline:
            ...
            result = self.runner.invoke(main, [
                ...
                '--vector-provider', 'qdrant_cloud',
                '--openai-api-key', 'sk-test',
                '--qdrant-cloud-url', 'https://test.qdrant.tech',
    ...
    def test_test_providers_command(self):
        ...
        with patch('src.providers.ProviderFactory') as mock_factory:
            ...
            mock_factory_instance.get_embedding_service.return_value = mock_embedding_service
            mock_factory_instance.get_vector_service.return_value = mock_vector_service
            mock_factory.return_value = mock_factory_instance
            
            result = self.runner.invoke(test_providers, [
                '--embedding-provider', 'openai',
                '--vector-provider', 'qdrant_cloud',
                '--openai-api-key', 'sk-test',
                '--qdrant-cloud-url', 'https://test.qdrant.tech',
    ...
    def test_migrate_providers_command(self):
        ...
        with patch('src.sync_pipeline.SyncPipeline') as mock_pipeline:
            ...
            with patch('click.confirm', return_value=True):  # Confirm migration
                result = self.runner.invoke(migrate_providers, [
                    '--from-embedding', 'ollama',
                    '--from-vector', 'qdrant_local',
                    '--to-embedding', 'openai',
                    '--to-vector', 'qdrant_cloud'
                ])
    ...
    def test_config_file_operations(self):
        ...
        from src.sync_pipeline import save_config_file, load_config_file
        ...
        with self.runner.isolated_filesystem():
            # Test saving config
            result = self.runner.invoke(save_config_file, [
                '--output', 'test-config.env'
            ], obj=config)
            ...
            # Test loading config
            result = self.runner.invoke(load_config_file, [
                '--config-file', 'test-config.env'
            ])
    ...
    def test_config_export_import(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(export_config, [
                '--format', 'json',
                '--output', 'config.json'
            ])
            ...
            # Import config
            result = self.runner.invoke(import_config, [
