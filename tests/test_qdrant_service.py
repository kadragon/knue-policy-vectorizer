...
from qdrant_client.models import (
    ...
    PointStruct, UpdateStatus, UpdateResult
)
from qdrant_client.http.exceptions import ResponseHandlingException
import numpy as np

from src.qdrant_service import QdrantService, QdrantError
...
class TestQdrantServiceErrorHandling:
    ...
    def test_upsert_operation_failure(self, mock_qdrant_client):
        ...
        mock_client = Mock()
        mock_qdrant_client.return_value = mock_client
        mock_client.upsert.side_effect = ResponseHandlingException("Server error")
    ...
    def test_delete_operation_failure(self, mock_qdrant_client):
        ...
        mock_client = Mock()
        mock_qdrant_client.return_value = mock_client
        mock_client.delete.side_effect = ResponseHandlingException("Server error")
...
class TestQdrantServiceIntegration:
    ...
    def test_full_lifecycle_workflow(self, mock_qdrant_client):
        ...
        mock_client.collection_exists.return_value = False
        mock_client.upsert.return_value = UpdateResult(operation_id=1, status=UpdateStatus.COMPLETED)
        mock_client.search.return_value = [
            Mock(id="doc1", score=0.95, payload={"title": "Document 1"})
        ]
        mock_client.delete.return_value = UpdateResult(operation_id=2, status=UpdateStatus.COMPLETED)
