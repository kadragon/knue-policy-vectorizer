...
def main():
    ...
    if not embedding_service.health_check():
        ...
        logger.info("💡 Please ensure Ollama is running: docker-compose -f docker-compose.qdrant.yml up -d")
        return False
...
"""
    ...
    try:
        ...
        logger.info(f"✅ Document validation: {'PASSED' if processed_doc['is_valid'] else 'FAILED'}")
        ...
        if len(doc_embedding) != 1024:
            logger.error(f"❌ Document embedding has wrong dimension: {len(doc_embedding)}")
            return False
    ...
    benchmark_texts = [
        "한국교원대학교 교육정책 " + str(i) * 50 for i in range(10)
    ]
    ...
    try:
        ...
        start_time = time.time()
        batch_embeddings = embedding_service.generate_embeddings_batch(benchmark_texts[:3])
        batch_time = time.time() - start_time
    ...
    logger.info(f"  - Batch processing efficiency: {(avg_single_time * 3 / batch_time):.2f}x")
